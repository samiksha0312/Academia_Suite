...
function AssignInstructor() {
  const { userName } = useParams(); // ✅ Get instructor userName from route
  ...
  // ✅ Assign selected courses to instructor
    ...
    try {
      await Promise.all(
        selectedCourses.map(async (courseId) => {
          ...
          return axios.post(
            "http://localhost:8080/auth/enrollment/assign/instructor/byUserName", // ✅ Instructor endpoint
            payload,
            {
      ...
      setMessage("✅ Courses assigned successfully to instructor!");
      setSelectedCourses([]);
    } catch (err) {
